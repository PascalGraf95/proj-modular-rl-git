...
class Actor:
    ...
    def update_history_with_latest_game_results(self, history_path, player_keys):
        ...
        game_result = self.get_side_channel_information('game_results')
        if game_result:
            self.append_to_game_results_history(self, game_result, history_path, player_keys)
            return True
        return False
    ...
    def append_to_game_results_history(self, game_result, history_path, player_keys):
        ...
        with open(history_path, 'a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([game_id, player_keys[0], player_keys[1], game_result[0], game_result[1], self.update_rating_period(path=history_path)])
    
    def get_current_rating_period(self, history_path):
        """
        ...
        # Check if there already exists a respective history file of played games
        if not os.path.isfile(history_path):
            raise Exception('No game history file found at {}'.format(history_path))
        else:
            # Check if the game history file contains any game results yet with pandas
            if pd.read_csv(history_path).empty:
                return 0
            else:
                # get the current rating period from the game results file using pandas
                rating_period = pd.read_csv(history_path)['rating_period'].iloc[-1]
            return rating_period
          
    def update_rating_period(self, path):
        """
        Update the rating period if each agent in the model path has an average of 10 games played in the last rating period.
        :param path: Path to the game results file
...
class Learner:
    ...
    def __init__(self, trainer_configuration, environment_configuration, model_dictionary=None,
        ...
        # - Model Weights -
        # Structures to store information about the given pre-trained models (and clone models in case of self-play)
    ...
    def load_checkpoint_by_mode(self, mode='latest', mode_two=None):
        # Try to load pre-trained models if provided. Otherwise, this method does nothing.
